...
let declaration_registration_code account_id type_app_id fun_app_id decl =
  ...
  | Dstructure s ->
      cpp_code_to_register_structures_and_record type_app_id account_id s
...
let main () =
  ...
  let cpp_code =
    ...
    ^ ( if contains output_file "/dosimetry" then
        "#include <dosimetry/release.hpp>\n"
      else "" )
    ^ ( if contains output_file "/cradle" then "#include <cradle/release.hpp>\n"
      else "" )
    ^ ( if contains output_file "/planning" then
        "#include <planning/release.hpp>\n"
      else "" )
    ^ ( if contains output_file "/maia" then "#include <maia/release.hpp>\n"
      else "" )
    ^ ( if contains output_file "/analysis" then
        "#include <analysis/release.hpp>\n"
      else "" )
    ^ ( if contains output_file "/dicom" then "#include <dicom/release.hpp>\n"
      else "" )
    ^ ( if contains output_file "/visualization" then
        "#include <visualization/release.hpp>\n"
      else "" )
    ^ "\n" ^ "namespace " ^ namespace ^ " {\n" ^ "\n"
    ^ String.concat ""
        (List.map
