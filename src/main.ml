...
let cpp_registration_code account_id type_app_id fun_app_id file_id decls =
  "void add_" ^ file_id ^ "_api(astroid::api_implementation& api)\n" ^ "{\n"
  ^ String.concat ""
      (List.map
...
let main () =
  ...
  let hpp_code =
    ...
    (* Let the core Astroid headers figure out their own dependencies. *)
    ^ ( if contains_substring output_file "astroid/typing/core" then ""
      else "#include <astroid/typing/core/preprocessed.h>\n" )
  ...
  let cpp_code =
    ...
    ^ "#include <astroid/typing/core/preprocessed.h>\n"
